...
$mn = mysql_real_escape_string(htmlspecialchars($_POST['mn']));
$date = mysql_real_escape_string(htmlspecialchars($_POST['date']));
}else{
$fio = mysql_real_escape_string(htmlspecialchars($_GET['fio']));
$phone = mysql_real_escape_string(htmlspecialchars($_GET['phone']));
$city = mysql_real_escape_string(htmlspecialchars($_GET['city']));
$adress = mysql_real_escape_string(htmlspecialchars($_GET['adress']));
$discount = mysql_real_escape_string(htmlspecialchars($_GET['discount']));
$mn = mysql_real_escape_string(htmlspecialchars($_GET['mn']));
...
mysql_query("INSERT INTO `ordermi` SET `fio`='$fio',`phone`='$phone',`city`='$city',`adress`='$adress',`discount`='$discount',`mn`='$mn',`date`='$date',`creator`='$uid'");
$ord = mysql_fetch_assoc(mysql_query("SELECT * FROM `ordermi` WHERE `phone`='$phone' ORDER BY id DESC LIMIT 1"));

header('Location: /work-worklist.php?sd=edit&id='.$ord[id].'');
exit;    
...
echo $mn; echo '<br />';
echo $date; echo '<br />';
    header('Location: /work-newwork.php');
    exit;  
}
...
if($w[block] == 1 and $uid > 2){
    header('Location: /work-worklist.php?sd=edit&id='.$id.'');
    exit;    
}
if($w[block] == 1 and $uid < 2 || $uid == 1){
    mysql_query("UPDATE `ordermi` SET `block`='0' WHERE `id`='$id'");
    header('Location: /work-worklist.php?sd=edit&id='.$id.'');
    exit;  
}
if($w[block] == 0){
    mysql_query("UPDATE `ordermi` SET `block`='1' WHERE `id`='$id'");    
    header('Location: /work-worklist.php?sd=edit&id='.$id.'');
    exit;  
}
break;
                    ...
                    <div class="col-sm-6">
                        <div class="nest" id="FootableClose">
                            ...
                            <div class="body-nest" id="Footable">
                                ...
                                <br /><br />
                                <form action='/ordermi-work.php?sd=pers-edit&id=<?echo $id?>' method='post'>
                                <table class="table-striped footable-res footable metro-blue" data-page-size="20" style="max-width: 500px;">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>ФИО</td>
                                            <td>
                                                <input type="text" name="fio" value="<? echo $w[fio] ?>" pattern="^[A-Za-zА-Яа-яЁё\s]+$" style="color: #000000 !important;" class="form-control">
                                            </td>    
                                        </tr>
                                        <tr>
                                            <td>Номер телефона</td>
                                            <td>
                                                <input type="text" name="phone" value="<? echo $w[phone] ?>" style="color: #000000 !important;" class="form-control">
                                            </td>    
                                        </tr>
                                        <tr>
                                            <td>Город</td>
                                            <td>
                                                <input type="text" name="city" value="<? echo $w[city] ?>" style="color: #000000 !important;" class="form-control">
                                            </td>
                                        </tr>    
                                        <tr>
                                            <td>Адрес</td>
                                            <td>    
                                                <input type="text" name="adress" value="<? echo $w[adress] ?>" style="color: #000000 !important;" class="form-control">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Скидка (%)</td>
                                            <td>
                                                <input type="text" name="discount" value="<? echo $w[discount] ?>" value="0" style="color: #000000 !important;" class="form-control">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Множитель</td>
                                            <td>
                                                <input type="text" name="mn" value="1" value="<? echo $w[mn] ?>" style="color: #000000 !important;" class="form-control">
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
...
$discount !== '' and
$mn !== ''){
mysql_query("UPDATE `ordermi` SET `fio`='$fio',`phone`='$phone',`city`='$city',`adress`='$adress',`discount`='$discount',`mn`='$mn' WHERE `id`='$id'");
...
echo $mn; echo '<br />';
echo $date; echo '<br />';
    header('Location: /ordermi-work.php?sd=pers-info&id='.$id.'');
    exit;  
}
...
case 'ordermi-phone':
mysql_query("UPDATE `ordermi` SET `hide`='1' WHERE `id`='$id'");
